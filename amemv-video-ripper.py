...
# Setting timeout 超时时间
TIMEOUT = 10

# Retry times 重试次数
RETRY = 5

# Numbers of downloading threads concurrently 线程数量
THREADS = 10
...
def download(medium_type, uri, medium_url, target_folder):
    """
    :param medium_type: 类型只能是图片或者视频
    :param uri: 子文件夹，专门用于存储此用户的，是此用户的id
    :param medium_url: 链接
    :param target_folder: 存储文件夹
    :return:
    """
    ...
    if os.path.isfile(file_path):
        remoteSize = getRemoteFileSize(medium_url)  # 远程文件大小，因为是视频或者图片会比较大
        localSize = os.path.getsize(file_path)  # 获取已经存在的文件的大小
        if remoteSize == localSize:  # 判断是否已经存在了
            return
    ...
    while retry_times < RETRY:
        try:
            ...
            with open(file_path, 'wb') as fh:   # 下载二进制文件（视频或者图片）
            ...
            pass
        retry_times += 1
    else: # 重试多次之后仍然失败，则会删除掉未下载成功的文件
        try:
            os.remove(file_path)  # 对于下载失败的文件，进行删除
        except OSError:
            pass
...
class DownloadWorker(Thread):  # 下载器
...
class CrawlerScheduler(object):  # 调度器
    ...
    def __init__(self, items):
        ...
        for i in range(len(items)):
            ...
            if not url: continue
            if re.search('share/user', url): self.numbers.append(url)   # 下载此用户的所有视频
    ...
    def _download_user_media(self, user_id, dytk, url):
        ...
        while True:
            ...
            if contentJson.get('has_more'): # 还有更多，则接着获取aweme
                max_cursor = contentJson.get('max_cursor')
            else:
                break
        if not noFavorite:  # 全局变量为False
